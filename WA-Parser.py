...
destination_directory = (
    "World-Anvil-Output"  # where you want the formatted files and folders to end up
)
    ...
    try:
        ...
        if response.status_code == 200:
            if not filename.lower().endswith(
                (".png", ".jpeg", ".jpg")
            ):  # I had one case where an image didn't get an extension .. never seen it again
                filename = (
                    filename + ".png"
                )  # Hoping and guessing its a png, i'll read bytes later
...
def format_content(content):
    ...
    text = re.sub(
        r"@\[([^\]]+)\]\([^)]+\)", r"[[\1]]", text
    )  # Replaces World Anvil links with Obsidian internal links
    text = re.sub(
        r"\r\n\r", r"\n", text
    )  # This was to fix some extra spacing issues that came from my export
    ...
    if attempt_bbcode:
        ...
        text = re.sub(
            r"\n +(\[h\d\])", r"\n\1", text
        )  # Remove leading spaces before headings
        text = re.sub(r"\[br\]", r"\n", text)  # [br] to newline
        text = re.sub(
            r"\[h1\](.*?)\[/h1\]", r"# \1", text
        )  # Convert [h1]...[/h1] to # ... (L1 heading)
        text = re.sub(
            r"\[h2\](.*?)\[/h2\]", r"## \1", text
        )  # Convert [h2]...[/h2] to ## ... (L2 heading)
        text = re.sub(
            r"\[h3\](.*?)\[/h3\]", r"### \1", text
        )  # Convert [h3]...[/h3] to ### ... (L3 heading)
        text = re.sub(
            r"\[h4\](.*?)\[/h4\]", r"#### \1", text
        )  # Convert [h4]...[/h4] to #### ... (L4 heading)
        text = re.sub(
            r"\[p\](.*?)\[/p\]", r"\1\n", text
        )  # Convert [p]...[/p] to a simple newline-delimited paragraph
        text = re.sub(
            r"\[b\](.*?)\[/b\]", r"**\1**", text
        )  # Convert [b]...[/b] to **...** (bold)
        text = re.sub(
        ...
        text = re.sub(
            r"\[u\](.*?)\[/u\]", r"<u>\1</u>", text
        )  # Convert [u]...[/u] to <u>...</u> (underline)
        text = re.sub(
            r"\[s\](.*?)\[/s\]", r"~~\1~~", text
        )  # Convert [s]...[/s] to ~~...~~ (strikethrough)
        text = re.sub(
            r"\[url\](.*?)\[/url\]", r"[\1]", text
        )  # Convert [url]URL[/url] to [text](URL)
        text = re.sub(
            r"\[list\](.*?)\[/list\]",
            lambda m: re.sub(r"\[\*\](.*?)\n?", r"* \1\n", m.group(1), flags=re.DOTALL),
            text,
            flags=re.DOTALL,
        )  # Convert [list]...[/list] to bullet point lists
        text = re.sub(
            r"\[code\](.*?)\[/code\]", r"```\n\1\n```", text
        )  # Convert [code]...[/code] to code blocks
        text = re.sub(
            r"\[quote\]([\s\S]*?)\[/quote\]", 
            lambda m: "\n> ".join(m.group(1).split("\n")),
            text,
            flags=re.DOTALL
        ) # Convert [quote] ... [/quote] to Obsidian block quotes
